...
namespace AditusBelli.Map
{
    ...
    public enum ResourceAmount { Scarce, Medium, Abundant }

    /// <summary>
    /// Shared procedural-generation engine. Holds the per-match inputs (seed, size,
    /// resources, players) and all the work — fractal terrain, painting, the
    /// <see cref="MatchLayout"/>, preview markers and camera framing. Subclasses only
    /// supply the terrain <see cref="Recipe"/> and which <see cref="ShapeWorldType"/> to
    /// shape as: <see cref="WorldMapGenerator"/> (game) reads the recipe from code,
    /// <c>WorldGenTuner</c> (sandbox) exposes every recipe parameter for hand tuning.
    /// </summary>
    public abstract class WorldGeneratorBase : MonoBehaviour
    {
        [Header("Seed")]
        ...
        public ResourceAmount resources = ResourceAmount.Medium;
        [Tooltip("1 human (slot 0) + up to 3 AI. One city center per player.")]
        [Range(2, 4)] public int playerCount = 2;

        // Supplied by subclasses.
        protected abstract WorldRecipe Recipe { get; }
        protected abstract WorldType ShapeWorldType { get; }
        protected virtual bool WantMarkers => false;     // preview markers (WorldGen only)
        protected virtual bool WantFitCamera => false;   // frame the whole map (WorldGen only)

        protected const int PreviewDim = 128;     // reduced resolution while live-tuning
        ...
        public void CopySettings()
        {
            ...
            Debug.Log("[WorldGenerator] Settings copied to clipboard.");
        }
        ...
        public void PasteSettings()
        {
            ...
            if (string.IsNullOrEmpty(json)) { Debug.LogWarning("[WorldGenerator] Clipboard is empty."); return; }
            JsonUtility.FromJsonOverwrite(json, this);
            Generate();
            Debug.Log("[WorldGenerator] Settings pasted from clipboard.");
        }
